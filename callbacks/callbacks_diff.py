...
def plot_differential_curve(n_clicks, pickup_str, bp1_str, slope1_str, bp2_str, slope2_str, unrestrained_str,
    ...
    try:
        ...
        if not list_ir: raise Exception("Falha ao gerar pontos da curva.")

        max_x_axis = max(list_ir[-1], test_ir, 1.0) * 1.1
        max_y_axis = max(list_iop[-1], test_iop, 1.0) * 1.2
        ...
        if max_x_axis > curve_ir_extended[-1]:
            ...
            curve_iop_extended.append(unres)

        op_zone_ir = curve_ir_extended + [max_x_axis, 0, 0]
        op_zone_iop = curve_iop_extended + [max_y_axis, max_y_axis, curve_iop_extended[0]]
        fig.add_trace(go.Scatter(
            x=op_zone_ir, y=op_zone_iop,
            fill='toself', mode='none', fillcolor='rgba(255, 50, 50, 0.2)', name='Zona de Operação'
        ))
        fig.add_trace(go.Scatter(
            x=curve_ir_extended, y=curve_iop_extended,
            fill='tozeroy', mode='lines',
            line=dict(color='cyan', width=3), name='Curva de Restrição',
            fillcolor='rgba(0, 156, 255, 0.3)'
        ))
        ...
        if test_iop >= unres:
            ...
            if iop_limite_no_ponto > unres: iop_limite_no_ponto = unres
        ...
        fig.add_trace(go.Scatter(
            x=[test_ir], y=[test_iop],
            mode='markers+text',
            marker=dict(color=cor_ponto, size=15),
            text=[f"Ponto de Teste<br>Status: {status_ponto}"],
            textposition="top right", name="Ponto de Teste"
        ))
        fig.update_layout(
            xaxis_range=[0, max_x_axis], yaxis_range=[0, max_y_axis],
            showlegend=False
        )
        return fig
